...
- `Box` is directed.
    - Recall, `*b` refers to the value in `b = Box::new(x)`.
    - `**b` does not refer to to `b`, rather, we would use `Box::new()` to go in the reverse direction.
...
- As a bonus: Write valid json. 
    - Web [tester](https://jsonformatter.curiousconcept.com/#).
    - [Or](https://askubuntu.com/a/1473153)
```sh
sudo apt install jq
jq <name-of-file>.json >/dev/null # should show nothing
```
