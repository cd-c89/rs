...
\begin{split}
   ...
   H_2^{(0)} = \texttt{0x3c6ef372fe94f82}, \quad H_3^{(0)} = \texttt{0xA54FF53A},\\
...
k = 1024 - (l + 129) % 1024
"l=", l, "k=", k, "pad=", "0x1" + "0" * (k//4) + f"{l:032x}" ## 128 // 4 == 32
```
...
- Addition is aproximately a binary bitwise operation:
- Equivalent to integer sum module $2^{64}$
```{python}
from ctypes import c_ulong ## this won't work in pypy
def addition(u:c_ulong, v:c_ulong) -> c_ulong:
    return c_ulong(u + v)

addition(4000000000, 4000000000) , addition(4, 4)
```
- Long used to mean 32 bit, Rust dodges this by being (1) new and (2) good.
    - Have to use (lol) `c_ulonglong`

## Rust Math
- Rust addition on `u64` is already modulo $2^{64}$
...
$$
\textsf{ROTR}^n(U) =  \underbrace{u_{64-n} u_{65-n} \cdots u_{62} u_{63}}_{n \textrm{ bits}} u_0 u_1 \cdots u_62-n} u_{63-n}
$$
